...
def remove_nils_and_false_from_array(array)
	array.reject {|element| element == nil || false}
end
...
array = [['Bob', 'Clive'], ['Bob', 'Dave'], ['Clive', 'Dave']]
...
def separate_array_into_even_and_odd_numbers(array)
	arr = array.select! {|element| element.odd?}
	arr << arra
end
...
def average_of_array(array)
	 ((array.inject(0) {|sum, element| sum + element.to_f})/array.count).round
end
...
def add_together_keys_and_values(hash)
	(hash.values.inject(0){|sum, value| sum + value}) + (hash.keys.inject(0){|sum, key| sum + key})
end
